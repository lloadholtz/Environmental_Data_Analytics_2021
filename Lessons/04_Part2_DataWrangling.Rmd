...
#choosing data from peter and paul lake from the nutrient dataset
...
#gets rid of the zero values when using code in line 47
summary(NTL.nutrient.data.PeterPaul$lakename)
...
#assign a new variable, function is pivot_longer, create a column called nutrient, second columnn called values_to 
...
#remove the rows that have NA in concentration column
...
#creating a list. specify a path to the folder where the files are located. pattern are the files you want to read. and we want to read the full names of the files.
LitterFiles

Litter <- LitterFiles %>%
  ldply(read.csv)
#use a pipe to read each element on that file
```
...
#use select function with the pipe
...
NTL.PeterPaul.summaries <- 
  ...
  summarise(meantemp = mean(temperature_C), 
            ...
            meanTP = mean(tp_ug), 
            sdTP = sd(tp_ug))
#filter by depth first, then group by lake name and month, filter NA for temp, and tn_ug, and tp_ug, use & so there is a row where none of these are there. summarize mean temp and sd, 
