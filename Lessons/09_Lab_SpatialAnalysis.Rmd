 ...
 * Spatial data structures: sfg, sfc, sf objects (has the geometry column, spatial dataframe)
...
huc8_UpperNeuse <- st_read('./Data/Spatial/NCHUC8.shp') %>%
  filter(SUBBASIN== 'Upper Neuse')


#Create a map
mapview(huc8_sf, col.regions='orange')+ 
  mapview(huc8_UpperNeuse, col.regions='purple')
...
wind_energy_huc8 <- read.csv('https://raw.githubusercontent.com/ENV859/EnviroAtlasData/main/Wind_Energy.csv', colClasses = c('HUC_12' ='factor'))%>% 
  ...
  group_by(HUC_8) %>%
  summarise(kWhkm2day=mean(kWhkm2day))
...
huc2_sf <- huc8_sf %>%
  group_by(DWQ_Basin)%>%
  summarise(ACRES=sum(ACRES), SQ_MILES=sum(SQ_MILES))

#Map the data

mapview(huc2_sf, zcol='ACRES')

#ggplot(huc2_sf)+ geom_sf()
