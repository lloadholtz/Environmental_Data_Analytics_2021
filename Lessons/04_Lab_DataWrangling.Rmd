...
#matrix sampling is extracting rows based on mask. leaving after comma blank means were grabbing every column
...
year1999_NTL.phys.data <- filter(NTL.phys.data, year4 == 1999)

# Exercise 2: 
# filter NTL.phys.data for Tuesday Lake from 1990 through 1999.

Tues90to99 <- filter(NTL.phys.data, lakename == 'Tuesday Lake', year4 %in% c(1990:1999))
#or we can use this code

Tues90to99two <- filter(NTL.phys.data, lakename == 'Tuesday Lake', year4 >= 1990, year4 <= 1999)

```
Question: Why don't we filter using row numbers?

> Answer: We dont use row numbers because its not as reproducable, also, it doesn't really tell us anything.. data sets change if you manipulate the data.
...
#summarize sqeeze all the rows, filtering all surface rows, and then we are taking the mean of all the surface rows. why we just have one result. its a summary statistic.
