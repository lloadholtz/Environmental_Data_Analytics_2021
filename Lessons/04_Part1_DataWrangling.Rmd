...
`<`
#values that are less than a number
`<=`
#check for values that are less than or equal to a number
`>`
`>=`
`&`
...
#creating a new dataset that is just showing samples with depth of 0

# dplyr filtering
NTL.phys.data.surface2 <- filter(NTL.phys.data, depth == 0)
#filtering the data so the depth is 0
...
#This is just looking at Paul lake and peter lake
...
#excluding the lakes we dont want here by using !=
NTL.phys.data.PeterPaul3 <- filter(NTL.phys.data, lakename %in% c("Paul Lake", "Peter Lake"))
#%in% selecting data on more than 2 elements. 
...
#specify a range for observations. day number is just an integer. 152 is first day of june, 305 is november 1 for June-October. Finding the observations within this range.
NTL.phys.data.JunethruOctober2 <- filter(NTL.phys.data, daynum > 151, daynum < 305)
#comma does the same thing as the &
NTL.phys.data.JunethruOctober3 <- filter(NTL.phys.data, daynum >= 152 & daynum <= 304)
#>= means the number itself will be included, so make the minor adjustments
NTL.phys.data.JunethruOctober4 <- filter(NTL.phys.data, daynum %in% c(152:304))
#%in% then insert the vector
...
#specify which column to arrange by
...
#allows us to choose certain columns in our dataset. the colon means through.
...
#left of the equal sign is the new column we are creating
...
#useful to help aggregate data by month by inserting a month column. left of equal sign is the new column.
...
#changing order of the column. lakeidk through daynum, then insert month, then sampledate through comments.
...
#function first and function last. 
```
