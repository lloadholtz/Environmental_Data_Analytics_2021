...
#there are no NAs in either of these datasets
...
Temperature_plot <- ggplot(BRA_complete, aes (x=Date, y=Temperature_C)) +
  geom_point() +
  geom_smooth(method=lm)

print(Temperature_plot)
#test for stationarity (testing if there is a trend or not) rainfall doesnt have much of a trend... mostly flat

Rainfall_plot <- ggplot(BRA_complete, aes (x=Date, y=Rainfall)) +
  geom_point() +
  geom_smooth(method=lm)
...
#frequency is 12 because they are collected monthly
#if it was daily, it would be 365
```
...
Temp_Components <- as.data.frame(Temperature_decomp$time.series[,1:3])
...
Temp_Components <- mutate(Temp_Components,
        Temperature = Temperature_decomp$Temperature_C,     
        Date = Temperature_decomp$Date)

Rainfall_Components <- mutate(Rainfall_Components,
        Rainfall = Rainfall_decomp$Rainfall,     
        Date = Rainfall_decomp$Rainfall)


BRA.Rainfall.plot <- ggplot(Rainfall_Components)+
  geom_line(aes(x=Date, y=Rainfall), color= "Grey")+
  geom_line(aes(x=Date, y=trend), color= "Red", size=1)

print(BRA.Rainfall.plot)

BRA.Temp.plot <- ggplot(Temp_Components)+
  geom_line(aes(x=Date, y=Temperature_C), color= "Grey")+
  geom_line(aes(x=Date, y=trend), color= "Red", size=1)
...
rain_data_trend1 <- Kendall::SeasonalMannKendall(Rainfall_ts)
