...
summary.factor(Neonics$Species.Common.Name)
#I tried using summary first. I did not think this was the best format because it categorized them alphabetically, and I would have to go through them to find what count was highest. For this many species to look through, it did not make sense to go about it this way.


sort(table(Neonics$Species.Common.Name))

#By using the sort feature, I was able to sort the species from lowest to highest. Scrolling to the bottom of the list, I am able to see the 6 most studied species. 
...
```{r}
ggplot(Neonics) + geom_freqpoly(aes(x=Publication.Year), bins=40, color= "limegreen")
```

10. Reproduce the same graph but now add a color aesthetic so that different Test.Location are displayed as different colors.
```{r}
ggplot(Neonics) +geom_bar(aes(x=Test.Location))
```
...
> Answer: There are 12 plots sampled at Niwot Ridge. Additionally, unique tells us how many unique values there are in this column. Summary gives us the total number of plot observations.
